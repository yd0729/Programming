...
#define REGISTER_MEMBERFUNCTION_CLASS(...)                                     \
    _REGISTER_MEMBERFUNCTIONS(NUM_ARGS(__VA_ARGS__), ##__VA_ARGS__)
...
#define _REGISTER_MEMBERFUNCTION_CLASSN(N, ...)                                \
    _REGISTER_MEMBERFUNCTION_CLASS##N(__VA_ARGS__)
#define _REGISTER_MEMBERFUNCTIONS(N, ...)                                      \
    _REGISTER_MEMBERFUNCTION_CLASSN(N, ##__VA_ARGS__)
/*
...
class LeetcodeInputHelper {
  public:
    ...
    LeetcodeInputHelper &operator>>(TreeNode *&ret) {
        ...
        while (true) {
            while (cin.isBlank(cin.getChar_Checked()))
                cin.next();
            if (getchar() == ']') {
                break;
            }
    ...
    template <typename T> LeetcodeInputHelper &operator>>(std::vector<T> &ret) {
        ret.clear();
        while (true) {
            char ch;
            cin >> ch;
...
class LeetcodeOutputHelper {
  public:
    LeetcodeOutputHelper() { cout.precision(5); }
...
struct _LeetcodeConstructorFactory : _LeetcodeFactoryBase<_Tp> {
    ...
    _Tp *operator()(std::tuple<std::decay_t<_Args>...> &__args,
                    std::index_sequence<__Is...>) {
    ...
    _Tp *operator()() override {
        ...
        if constexpr (_IsSolution) {
            std::apply(
                [&](auto &&...args) { ((LeetcodeInputHelper() >> args), ...); },
                m_argument);
        } else {
            cin >> ch;
            assert(ch == '[');
            std::apply(
                [&](auto &&...args) {
                    ((LeetcodeInputHelper() >> args, cin >> ch), ...);
                },
                m_argument);
...
struct _LeetcodeMemberFunction : public _LeetcodeMemberFunctionBase<_Tp> {
    ...
    template <size_t... __Is>
    void exec(_Tp *__obj, std::tuple<std::decay_t<_Args>...> &__args,
              std::index_sequence<__Is...>) {
...
class LeetcodeExecutor {
    ...
    std::unordered_map<std::string, _LeetcodeMemberFunctionBase<_Tp> *>
        m_funcMap;
    _Tp *m_obj = nullptr;
  ...
  public:
    ...
    void registerMemberFunction_ofSolution(const char *name,
                                           R (_Tp::*func)(_Args...)) {
        m_funcMap[name] =
            new _LeetcodeMemberFunction<true, _Tp, R, _Args...>(func);
    }

    template <typename R, typename... _Args>
    void registerMemberFunction_ofClass(const char *name,
                                        R (_Tp::*func)(_Args...)) {
        m_funcMap[name] =
            new _LeetcodeMemberFunction<false, _Tp, R, _Args...>(func);
    }

    void construct() { m_obj = (*m_constructor)(); }
    ...
    explicit operator bool() { return m_cursor < m_commands.size(); }
};
} // namespace OY
